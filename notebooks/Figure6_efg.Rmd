...
```{r}
files <- list.files("../results/logs/", pattern = "metadata.tsv", full.names = TRUE)
...
p_all <- plot_auc %>%
  ...
  scale_fill_manual(values = c("EM" = "#b2182b",
                               "LR with sl" = "#91bfdb",
                               "LR" = "#2166ac")) +
...
p_PWM_EM <- plot_auc %>%
  mutate(model = recode(model,
                        ...
                        "Logistic regression" = "LR",
                        "Logistic regresion with acc info" = "LR with sl")) %>%
  filter(model %in% c("EM (RAD-seq)", "PWM (control)")) %>% 
  ggplot(aes(x = model, y = AUC, fill = model)) +
  geom_boxplot(outlier.shape = NA, width = 0.3, color = "black") +
  
  geom_line(aes(group = file_title), color = "gray60", alpha = 0.8, linewidth = 0.1) + 
  
  geom_jitter(width = 0.12, alpha = 0.6, size = 1.8, stroke = 0, show.legend = FALSE) +
  stat_summary(fun = mean, geom = "point", shape = 23, size = 3, fill = "white", color = "black") +
  scale_fill_manual(values = c("EM (RAD-seq)" = "#b2182b",
                               "LR with sl" = "#91bfdb",
                               "LR" = "#2166ac")) +
  labs(x = NULL, y = "AUC", title = "Supervised LR vs. unsupervised EM model") +
  theme_minimal(base_size = 24) +
...
p_PWM_EM
ggsave("../../final_figures/Fig6f_EM_PWM_model_comparison.pdf", width = 5, height = 5)
```
...
ggsave("../../final_figures/Fig6e_AUC_scatterplot_of_all.pdf")
```
...
```{r}
coef_dir   <- "../logs/"        # folder holding <sample>_beta_coefficients.tsv
out_prefix <- "./routput/r_output"  # prefix for saved plots

# --- 1) Read all coefficient files ---
coef_files <- list.files(coef_dir, pattern = "_beta_coefficients\\.tsv$", full.names = TRUE)
read_coef <- function(f) {
  read_tsv(f, show_col_types = FALSE) %>%
    mutate(sample = str_remove(basename(f), "_beta_coefficients\\.tsv$")) %>% 
    mutate(sample = str_remove(basename(f), "_beta_coefficients_s_l\\.tsv$"))
}
...
coef_files <- list.files(coef_dir, pattern = "_beta_coefficients_s_l\\.tsv$", full.names = TRUE)
...
coef_all <- rbind(coef_all, coef_s_l) %>% mutate(sample = gsub("_beta_coefficients.tsv", "", sample))
...
ggsave("../../final_figures/Fig6g_beta_relative_distributioin.pdf", plot = p_box_l2, device = cairo_pdf, width = 6, height = 4)
```
